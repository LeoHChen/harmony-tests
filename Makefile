...
linux_binary_upload_path := ${linux_base_upload_path}/harmony-tests
linux_testcases_upload_path := ${linux_base_upload_path}/testcases.tar.gz

dist := ./dist/harmony-tests
env := GO111MODULE=on
DIR := ${CURDIR}
...
upload-linux:package-testcases static
	aws s3 cp dist/harmony-tests ${linux_binary_upload_path} --acl public-read
